...
\label{delJ}
\end{equation}
This is the general form of the divergence of the current density \tjs{at each node}. \tjsr{These}{This} set of equations can be \tjsr{placed}{formulated} into a matrix \tjs{equation} which is a linear function of \tjs{the} particle densit\tjsr{y}{ies} at time t,
\begin{equation}
...
\label{tdr}
\end{equation}
\tjs{D}{The d}ielectric relaxation time is only important when the electric potential is significantly affected by the redistribution of charge over time. Otherwise it has minimal impact on the stability of the problem.
