...
Three different analytic solutions to drift diffusion equations are developed in this section. \tjsr{It starts with}{Initally} a simple steady state problem with uniform electric field \tjs{is solved}. The second example, which is more complex than the first one, is a transient solution for a charge distribution moving under uniform electric field. Finally, a PN junction problem is solved which incorporates the drift diffusion equation with Poisson's equation and provides a steady state solution for electron/hole density distributions, electric potential and electric field. 
...
Equation \ref{p_ss} combined with equation \ref{p_ss_c} gives a complete solution for this problem. \tjsr{which}This solution shows that increasing the electric field will \tjsr{concentrate}{increase} the \tjs{concentration of} charge density at the edge of the conductor. Physically this behavior is reasonable, since the force that is pushing the particles against a wall is getting greater and it is making the charges accumulate even more. \tjs{The F}{f}ollowing plot demonstrates the affect of the electric field on the hole distribution.
\begin{figure}[!htp]
\centering
...
\frac{\partial n}{\partial t} = \frac{1}{q}\nabla \cdot (\vec{J_n})
\end{equation}
\tjs{Substituting in the drift-diffusion expression for $J_n$ we obtain,}
\begin{equation}
